...
if (!empty($_FILES['image'])) {
    ...
    if ($fileSize < 5000000) {
        if ($data['image'] !== "") {
            $file_with_path = $_SERVER['DOCUMENT_ROOT'] . '/vanilla/taniver-game/' . $data['image'];
...
if ($auth->isValid()['success'] === 1) {
    if (
        ...
        isset($data['link']) && isset($data['video_link']) &&
        isset($data['genre_id']) && isset($data['user_id'])
    ) {
        $game->id = $data['id'];
        ...
        if ($game->update()) {
            echo json_encode(
                array('success' => 1, 'status' => '201', 'message' => 'game updated')
            );
        } else {
            echo json_encode(
                array('success' => 0, 'status' => '500', 'message' => 'game not updated')
            );
        }
    } else {
        echo json_encode(
